...
unsigned long Led,SW1_last,SW2_last,SW1_new,SW2_new,count = 0;
...
int main(void){  unsigned long i,last,now;
  ...
  last = NVIC_ST_CURRENT_R;
	...
	SW1_last = GPIO_PORTF_DATA_R & 0x10;
	SW2_last = GPIO_PORTF_DATA_R & 0x01; 
  EnableInterrupts();           // enable interrupts for the grader
  while(1){
		SW1_new = GPIO_PORTF_DATA_R & 0x10;
	  SW2_new = GPIO_PORTF_DATA_R & 0x01;
    Led = GPIO_PORTF_DATA_R & 0x02;   // read previous
		if (SW1_new == 0x00 || SW2_new == 0x00)
		{
    ...
    GPIO_PORTF_DATA_R = Led;   // output 
		if(Led != GPIO_PORTF_DATA_R || SW1_last != SW1_new || SW2_last != SW2_new)
		{
			if(i<50){
